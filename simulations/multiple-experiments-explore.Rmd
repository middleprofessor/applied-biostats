...
simulator <- function(
  ...
  # nested design
  Z1 <- model.matrix(~ 0 + id, data = fake_data_all)
  Z2 <- model.matrix(~ 0 + id:treatment, data = fake_data_all)
  Z <- cbind(Z1, Z2)

  # random intercepts and slopes
  Z1 <- model.matrix(~ 0 + id, data = fake_data_all)
  Z2 <- model.matrix(~ 0 + treatment, data = fake_data_all)
  Z <- cbind(Z1, Z2)
...
simulator_unequal <- function(
  ...
  for(sim_i in 1:n_sim){
    ...
    # lForm <- lFormula(fake_y ~ treatment + (treatment | id),
  ...
  for(sim_i in 1:n_sim){
    ...
    fake_data_all[, c("genotype","treatment", "id") :=
                    tstrsplit(tc.by.id,
                              " ",
                              fixed = TRUE)]
    g_by_t <- factor(paste(fake_data_all$genotype, fake_data_all$treatment),
                                     levels = g.by.t_levels)
    sigma_vec <- sigma[as.integer(g_by_t)]
    # fake_data_all[, g_by_t := factor(paste(genotype, treatment),
    #                                  levels = g.by.t_levels)]
    ...
    Z <- model.matrix(~ 0 + id + id:genotype + id:treatment +
                      ...
                      data = fake_data_all)
    # colnames(Z) <- 1:ncol(Z)
    # colnames(Z1) <- 1:ncol(Z1)
    # cbind(fake_data_all, Z, Z1)

    # attempt at type 3 like random effects fails
    # Z1 <- copy(Z)    
    # for(i in 1:n_id){
    #   j <- seq(n_id+i,n_id*2+i, n_id)
    #   Z1[Z1[,i] == 1, j] <- Z1[Z1[,i] == 1, j] - 0.5
    # }
    # block2 <- (n_id*1+1):(n_id*2)
    # block3 <- (n_id*2+1):(n_id*3)
    # block4 <- (n_id*3+1):(n_id*4)
    # Z1[, block4] <- Z1[, block2] * Z1[, block3]
...
for(i in 1:n_id){
  j <- seq(n_id+i,n_id*2+i, n_id)
  Z[Z[,i] == 1, j] <- Z[Z[,i] == 1, j] - 0.5
}
block2 <- (n_id*1+1):(n_id*2)
block3 <- (n_id*2+1):(n_id*3)
block4 <- (n_id*3+1):(n_id*4)
Z[, block4] <- Z[, block2] * Z[, block3]
Z
```


```{r eval=FALSE}
fake_data <- fake_data_all_orig[, 1:4]
fake_data[, g_by_t := paste(genotype, treatment)]
fake_matrix <- fake_data_all_orig[, -(1:4)] %>%
  as.matrix()
sim_i <- 1
fake_data[, y := fake_matrix[, sim_i]]
m0 <- lmer(y ~ genotype + treatment +
             (genotype + treatment | id),
           data = fake_data)
VarCorr(m0)

fake_data <- fake_data_all1[, 1:4]
fake_data[, g_by_t := paste(genotype, treatment)]
fake_matrix <- fake_data_all1[, -(1:4)] %>%
  as.matrix()
sim_i <- 1
fake_data[, y := fake_matrix[, sim_i]]
m0 <- lmer(y ~ genotype + treatment +
...
simulate_this_aov <- function(
  ...
  for(sim_i in 1:n_sim){
    fake_data[, y := fake_matrix[, sim_i]]
    fake_data[, .(sd = sd(y)), by = treatment]
    fake_means <- fake_data[, .(y = mean(y)),
                            by = .(treatment, id)]
...
simulate_this_rmaov <- function(
  ...
  for(sim_i in 1:n_sim){
    ...
    m1 <- aov_4(y ~ treatment + (treatment|id),
                data = fake_data,
                include_aov = TRUE)
    m2 <- aov_4(y ~ treatment + (treatment|id),
                data = fake_data,
                include_aov = FALSE)
    # lmm random intercept
    m3 <- lmer(y ~ treatment + (1|id),
               data = fake_data)
    ...
    res1$anova <- anova(m1, correction = "none")[1, "Pr(>F)"]
    res2$anova <- anova(m1, correction = "GG")[1, "Pr(>F)"]
    res3$anova <- anova(m3, ddf="Kenward-Roger")[1,"Pr(>F)"]
    ...
    error_mat[sim_i, "aov4-uni"] <- res1$p_value
    error_mat[sim_i, "aov4-multi"] <- res2$p_value
    error_mat[sim_i, "intercept"] <- ifelse(is.null(lmer_check(m3)), res3$p_value, NA)
  }
  return(list(p_mat = p_mat,
              error_mat = error_mat,
              lower_mat = lower_mat,
...
```{r}
simulate_this_rmaov_factorial <- function(
  seed_i = 1,
  n_sim = 2000,
  n = 1,
  n_id = 5,
  ...
  sigma = c(1,1,1,1),
  gamma = c(1,1,1,1),
  rho = rep(0.6, 6), # r between random intercept and slope
  do_anova = FALSE,
  equal_n = TRUE
){
  ...
  fake_data <- all_data[, 1:4]
  fake_data[, g_by_t := paste(genotype, treatment)]
  fake_matrix <- all_data[, -(1:4)] %>%
    as.matrix()
  for(sim_i in 1:n_sim){
    fake_data[, y := fake_matrix[, sim_i]]
    # m0 <- lmer(y ~ genotype * treatment +
    ...
    m3 <- aov_4(y ~ g_by_t +
                ...
                include_aov = TRUE)
    # lmm random intercept
    m5 <- lmer(y ~ genotype * treatment +
                 (1|id),
               data = fake_data)
    m6 <- lmer(y ~ genotype * treatment +
                 (1|id) +
                 (1|id:genotype) +
                 (1|id:treatment),
               data = fake_data)
   # lmm correlation
    m7 <- safelme(y ~ genotype * treatment,
              random = ~1 | id,
              correlation = corSymm(form = ~ 1 | id),
              data = fake_data)$result
    m8 <- safelme(y ~ genotype * treatment,
              random = ~1 | id,
              weight = varIdent(form = ~ 1 | g_by_t),
              data = fake_data)$result
    m9 <- safelme(y ~ genotype * treatment,
              random = ~1 | id,
              correlation = corSymm(form = ~ 1 | id),
              weight = varIdent(form = ~ 1 | g_by_t),
              data = fake_data)$result
    ...
    res2 <- contrast_stats(m1,
                            specs = c("genotype", "treatment"),
                            model = "multivariate")
    res3 <- contrast_stats(m3, specs = c("g_by_t"))
    res4 <- contrast_stats(m3,
                            specs = c("g_by_t"),
                            model = "multivariate")
    res5 <- contrast_stats(m5, specs = c("genotype", "treatment"))
    res6 <- contrast_stats(m6, specs = c("genotype", "treatment"))
    if(!is.na(m7[1])){
      fit_pairs <- emmeans(m7, specs = c("genotype", "treatment"), data = fake_data) %>%
        contrast(method = "revpairwise",
                 adjust = "none") %>%
        summary(infer = TRUE)
      res7$lower <- fit_pairs[1, "lower.CL"]
      res7$upper <- fit_pairs[1, "upper.CL"]
      res7$p_value <- fit_pairs[1, "p.value"]
    }else{
      res7$lower <- NA
      res7$upper <- NA
      res7$p_value <- NA
    ...
    if(!is.na(m8[1])){
      fit_pairs <- emmeans(m8, specs = c("genotype", "treatment"), data = fake_data) %>%
        contrast(method = "revpairwise",
                 adjust = "none") %>%
        summary(infer = TRUE)
      res8$lower <- fit_pairs[1, "lower.CL"]
      res8$upper <- fit_pairs[1, "upper.CL"]
      res8$p_value <- fit_pairs[1, "p.value"]
    }else{
      res8$lower <- NA
      res8$upper <- NA
      res8$p_value <- NA
    ...
    if(!is.na(m9[1])){
      fit_pairs <- emmeans(m9, specs = c("genotype", "treatment"), data = fake_data) %>%
        contrast(method = "revpairwise",
                 adjust = "none") %>%
        summary(infer = TRUE)
      res9$lower <- fit_pairs[1, "lower.CL"]
      res9$upper <- fit_pairs[1, "upper.CL"]
      res9$p_value <- fit_pairs[1, "p.value"]
    }else{
      res9$lower <- NA
      res9$upper <- NA
      res9$p_value <- NA
    ...
    res1$anova <- anova(m1, correction = "none")[1, "Pr(>F)"]
    res2$anova <- anova(m1, correction = "GG")[1, "Pr(>F)"]
    res3$anova <- anova(m3, correction = "none")[1, "Pr(>F)"]
    res4$anova <- anova(m3, correction = "GG")[1, "Pr(>F)"]
    res5$anova <- anova(m5, ddf = "Kenward-Roger")[1,"Pr(>F)"]
    res6$anova <- anova(m6, ddf = "Kenward-Roger")[1,"Pr(>F)"]
    res7$anova <- NA
    res8$anova <- NA
    res9$anova <- NA
    ...
    error_mat[sim_i, "aov4-fact-uni"] <- res1$p_value
    error_mat[sim_i, "aov4-fact-multi"] <- res2$p_value
    error_mat[sim_i, "aov4-flat-uni"] <- res3$p_value
    error_mat[sim_i, "aov4-flat-multi"] <- res4$p_value
    error_mat[sim_i, "intercept"] <- ifelse(is.null(lmer_check(m5)), res5$p_value, NA)
    error_mat[sim_i, "intercept_ixn"] <- ifelse(is.null(lmer_check(m6)), res6$p_value, NA)
    error_mat[sim_i, "corsymm"] <- res7$p_value
    error_mat[sim_i, "weights"] <- res8$p_value
    error_mat[sim_i, "sym_weight"] <- res9$p_value
  }
  return(list(p_mat = p_mat,
              error_mat = error_mat,
              lower_mat = lower_mat,
...
beta_1_list <- c(0)
sigma <- 1
gamma_0_list = c(0.2, 1, 2) # intercept 
gamma_1_list = c(0.2, 1) # slope
gamma_2_list = c(0.2, 1) # slope
...
```{r run factorial RMANOVA, message=FALSE, warning = FALSE, eval = FALSE}
...
sigma <- c(1,1,1,1)
#sigma <- c(0.5, 2, 1, 2)
gamma_0_list = c(0.2, 2) # intercept 
gamma_1_list = c(0.2, 1) # slope genotype
gamma_2_list = c(0.2, 1) # slope treatment
gamma_3_list = c(0.2, 1) # interaction
rho_list <- c("0", "+") # correlation between random intercept and slope
# rho_list <- c("+") # correlation between random intercept and slope
equal_n_list <- c(TRUE) # equal_n_list
rho_vec <- rep(0.6, 4*3/2)

parameter_mat <- expand.grid(beta_1 = beta_1_list,
                             gamma_0 = gamma_0_list,
                             gamma_1 = gamma_1_list,
                             gamma_2 = gamma_2_list,
                             gamma_3 = gamma_3_list,
                             rho = rho_list,
                             equal_n = equal_n_list
                             ) %>%
  data.table

parameter_mat <- parameter_mat[c(1,32),]
...
for(world in 1:nrow(parameter_mat)){
  ...
  gamma_1 <- parameter_mat[world, gamma_1]
  gamma_2 <- parameter_mat[world, gamma_2]
  gamma_3 <- parameter_mat[world, gamma_3]
  if(parameter_mat[world, rho] == "0"){
    rho <- rep(0, 4*3/2) # r2c1, r3c1,r3c2
  }
  if(parameter_mat[world, rho] == "+"){
    rho <- rho_vec # r2c1, r3c1,r3c2
  }
  ...
  # rm or not?
  sim1 <- simulate_this_rmaov_factorial(
    seed_i = seed_i,
    n_sim = n_sim,
    n = n,
    n_id = n_id,
...
for(i in 1:nrow(parameter_mat)){
  lower <- big_table[stat == "lower" &
                       ...
                       gamma_0 == parameter_mat[i, gamma_0] &
                       gamma_1 == parameter_mat[i, gamma_1] &
                       gamma_2 == parameter_mat[i, gamma_2] &
                       rho == parameter_mat[i, rho] &
  ...
  anova_less_than <- apply(anova_p, 2, function(x) mean(x < 0.05, na.rm = TRUE))
  
  anovaGG_less_than <- apply(anovaGG_p, 2, function(x) mean(x < 0.05, na.rm = TRUE))
...
# error_less_than_table
# anova_less_than_table
which_table <- error_less_than_table 

which_table <- which_table[, sim := .I]
which_table_long <- melt(which_table,
                      measure.vars = methods,
                      variable.name = "method",
                      value.name = "prob")

x_labels <- x_labeller(which_table_long[, 2:6])

gg_type_1 <- ggplot(data = which_table_long[beta_1 == 0],
                   aes(x = sim,
                       y = prob,
                       color = method)) +
  geom_line() +
  ...
  #                 ylim = c(0, .2)) +
  coord_cartesian(ylim = c(0.01, .08)) +
