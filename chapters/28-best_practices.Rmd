...
```{r, echo=FALSE, warning=FALSE, message=FALSE, fig.cap="Distribution of the counts in the wildtype (WT) and sox10 knockout (sox10-) groups. Both groups show a strong right skew, which is common with count data."}
folder <- "Data from The enteric nervous system promotes intestinal health by constraining microbiota composition"
filename <- "journal.pbio.2000689.s008.xlsx"
...
summary(contrast(m1_emm, method="revpairwise"), infer=c(TRUE, TRUE))
```

### Bootstrap Confidence Intervals

A bootstrap confidence interval is computed from the distribution of a statistic from many sets of re-sampled data. The basic algorith is

1. compute the parameter for the observed data, assign this to $\theta_1$
...
3. compute the parameter for the resampled data, assign these to $\theta_{2..m}$
4. repeat 2 and 3 $m-1$ times
5. given the distribution of $m$ parameter estimates, compute the lower interval as the $\frac{\alpha}{2}$th percentile and the upper interval as the $1 - \frac{\alpha}{2}$th percentile. For 95% confidence intervals, these are the 2.5th and 97.5th percentiles.

The focal parameter in the Fig. 4A neutrophil count data is the difference in the mean count for the sox10 and wild type groups (the coefficient for $treatment$ in the linear model). The script below, which computes the 95% confidence intervals of this difference, resamples *within* each group to preserve the original sample size within each group.
...
```{r}
boot_diff <- function(data, indices, n_resamples){
  dt <- data[indices]
}
...
```{r}
coef(summary(lm(log(count + 1) ~ treatment, data=fig4a)))
