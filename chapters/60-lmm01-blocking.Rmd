...
all_treatments_per_batch <- function(sigma=1, sigma_b0=1, sigma_b1=1, beta_0=10, beta_1=1, n_block=6, n_subsamp=10, Y_label="Y", trt_levels=c("Cn","Tr"), block_label="block", confound=FALSE){
...
With the random factor in the column "block:

1. add a random intercept using `y ~ treatment + (1|block)`
2. add a random slope and intercept using  `y ~ treatment + (treatment|block)`
...
A warning that "Model failed to converge with 1 negative eigenvalue" does mean there is a problem. A solution is to simply the model (remove a random slope if one is specified) 
