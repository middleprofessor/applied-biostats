...
1. We are using the function `glmmTMB`, which is allows fitting from multiple distribution families. Other than the addition of the `family` argument, the R code looks like that for the `lm` fit.
...
1. Generalized linear models are covered more thoroughly in [Chapter](#counts).
...
by_list <- c("sim", "normal","type", "n", "theta1", "theta2")
...
```{r violations-glm-count-simulation-plot, echo=FALSE, fig.caption = "(A) histogram of distribution used for simulation of non-normal count response (sampled from negative binomial distribution with mu and theta equal to the observed values in Fig. 6f). (B) An example of a sampled fake data set."}
n <- 5000
count <- rnegbin(n,
              mu = 3633140,
              theta = 6.6)
