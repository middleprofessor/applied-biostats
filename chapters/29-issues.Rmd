...
Reproducibility blues. Researchers are improving best practices by archiving data on public servers. Nevertheless, it is extremely rare to find the full data for many experiments. For analyses of the means from multiple experiments, researchers almost always archive the means and not the full set of measurements from each experiment. There is no way to replicate a result if only the means are archived. There is also no way to re-analyze the data with alternative models that require the full data.
</div>

### Multiple experiments Example 1 (wound healing Exp4d) {#issues-exp4d}
The data for this section is from the experiment in Figure 4d of
...
```{r twoway-exp4d-import, echo = FALSE}
...
#### Linear mixed model for combining replicated experiments {#issues-combined-experiments}

The linear mixed model for combining replicated experiments is most easily fit using the `aov_r` function from the afex package

**fit the model**

```{r  twoway-exp4d-m1, warning=FALSE, message=FALSE}
...
```{r  twoway-exp4d-m1-pairs, warning=FALSE, message=FALSE}
...
1. Model `exp4d_m1` adds $\texttt{experiment_id}$ as a random factor but the formula is misleading. The formula specifies a linear mixed model with a random intercept and a random slope (see [Models with random factors -- Blocking and pseudoreplication](#lmm}). The actual model fit by `aov_4` is equivalent to a **nested model**, which I'll get to in item 3.
2. To understand the nested model, start with a factorial linear model with $\texttt{genotype}$ and $\texttt{experiment_id}$ crossed. This is the **fixed-effects** model that is the focus of this chapter.

$$
\begin{align}
\texttt{vimentin_cells}_{j} = &\ \beta_0 + \beta_1 (\texttt{genotype}_\texttt{STAT3-/-}) + \beta_{2j} (\texttt{experiment_id}_{j})\ + \\
&\beta_{3j} (\texttt{genotype}_\texttt{STAT3-/-}:\texttt{experiment_id}_{j}) + \varepsilon
\end{align}
$$

In this model, there are $\beta_2$ and $beta_3$ coefficients for all non-reference levels of $\texttt{experiment_id}$. The R formula for this is `vimentin_cells ~ genotype * experiment_id`. Do not fit this model -- it fails to properly model the correlated error and can be conservative under some kinds of correlated error and highly anti-conservative under other kinds of correlated error (Simulation xxx).

3. The nested model is

$$
\texttt{vimentin_cells} = \beta_0 + \beta_{0j} + \beta_{0jk} + \beta_1 (\texttt{genotype}_\texttt{STAT3-/-}) + \varepsilon
$$

$\beta_0$ and $\beta_1$ have the usual interpretation. $\beta_1$ is the effect that we want to estimate.

$\beta_{0j}$ and $\beta_{0jk}$ are **random intercepts** from adding $\texttt{experiment_id}$ as a random (and not fixed) factor.

$\beta_{0j}$ is the random intercept for the *j*th experiment and models the correlation due to shared features within experiments. It is similar to $\beta_{2j}$ in the fixed effect model except that there is a coefficient for all levels of $\texttt{experiment_id}$.

$\beta_{0jk}$ is the random intercept for all combinations of the *j*th experiment with the *k* level of $\texttt{genotype}$. It is similar to $\beta_{3j}$ in the fixed effect model except that there is a coefficient for all combinations and not just the non-reference combinations.

4. The nested model is implemented using `lmer` from the package lme4.

```{r twoway-exp4d_m2, warning = TRUE}
exp4d_m2 <- lmer(vimentin_cells ~ genotype +
                         (1|experiment_id) +
...
Oops. The function fits the model but returns a **convergence warning**, meaning that we shouldn't use the fit for inference. Convergence warnings will be very common with replicated experimental data for several reasons but the main reason is that there aren't enough replicates (three is the most common) to estimate the variance among the experiments. The only reason we'd want to use this function over the `aov_4` function is if we wanted these variance estimates.

5. An equivalent of the nested model, at least for estimating the treatment effects and the uncertainty of these effects, is a simple linear mixed model of the Experiment means. These are the means typically used by researchers to compute a *t*-test or ANOVA.


```{r twoway-exp4d-means}
...
```{r twoway-exp4d-m3, message=FALSE}
exp4d_m3 <- lmer(vimentin_cells ~ genotype +
                              (1|experiment_id),
                            data = exp4d_means)

exp4d_m3_emm <- emmeans(exp4d_m3, specs = "genotype")
exp4d_m3_pairs <- contrast(exp4d_m3_emm,
                     method = "revpairwise") %>%
  summary(infer = TRUE)
...
6. The CI computed by `emmeans::contrast` uses the Kenward-Roger df and will be a little wider than the CI computed using `confint`.
7. There is really no reason to use this instead of the `aov_4` function (`exp4d_m1`). Models like `exp4d_m2` are susceptible to convergence warnings for replicated experiments because of the low number of replicates. `aov_4` function avoids these fit issues by using a multivariate model and not a linear mixed model to compute the inferential statistics.

#### A *t*-test or ANOVA of the means has less power

Compare the nested model in `exp4d_m1` or `exp4d_m3` (these are the same) to a *t*-test/ANOVA of the experiment means

```{r twoway-exp4d-ttest}
exp4d_m4 <- lm(vimentin_cells ~ genotype, data = exp4d_means)

exp4d_m4_coef <- cbind(coef(summary(exp4d_m4)),
                 confint(exp4d_m4))
exp4d_m4_coef %>%
  kable(digits = c(2,3,1,5,2,2)) %>%
  kable_styling()
```

Notes

1. Remember that a *t*-test of experiment means is equivalent to the linear model `exp4d_m4`.
2. The SE for the estimate of the $\texttt{genotype}$ effect is much smaller for the random intercept model than for the model without the random intercept (*t*-test).
3. A consequence of item 1 is the narrower CI for the estimate of $\texttt{genotype}$ in the random intercept model.
4. A consequence of item 1 is the smaller *p*-value for the estimate of $\texttt{genotype}$ in the random intercept model (look closelyy because a quick glance might suggest they are almost they same. They aren't).
5. The increased precision and power is a general result and not one specific to this example.

#### A paired *t*-test is a special case of a nested model

```{r twoway-nested-model-paired-t}
exp4d_m5 <- t.test(vimentin_cells ~ genotype,
               paired = TRUE,
           data = exp4d_means)
tidy(exp4d_m5) %>%
  kable(digits = c(2,1,5,1,2,2,1,1)) %>%
  kable_styling()
...
1. The signs of the effect and confidence intervals are reverse of those in the nested model because the `t.test` function generates the contrast as (reference - treatment).

#### Two-way mixed ANOVA

The nested model (`exp4d_m1`, `exp4d_m2`, `exp4d_m3`) is equivalent to a two-way ANOVA with one fixed factor and one random factor (**two-way mixed ANOVA**).

```{r twoway-nested-model-anova}
anova(exp4d_m1)[,-5] %>%
  kable() %>%
  kable_styling()
```
...
Source: [Innervation of thermogenic adipose tissue via a calsyntenin 3β–S100b axis](https://idp.nature.com/authorize/casa?redirect_uri=https://www.nature.com/articles/s41586-019-1156-9&casa_token=vnFIQyig4UoAAAAA:3hBrakzgGdZ7hYaUBmTHj7dyD2PxQp6iCQXmTJNeFQnIETrRhg_gZwVDPwteGOOf3sFxS9M_Tfa45w4t)
...
```{r twoway-exp4d-import-show, echo = TRUE}
data_from <- "Distinct inflammatory and wound healing responses to complex caudal fin injuries of larval zebrafish"
file_name <- "elife-45976-fig4-data2-v1.xlsx"
file_path <- here(data_folder, data_from, file_name)
