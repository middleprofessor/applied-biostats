...
here::here() creates an absolute path, but one that is created on the fly, and will change (or should change) correctly if the project folder is moved on the same machine or to another machine. And no chunk named setup is required so this also works in every R markdown file when using Bookdown.
...
Start each notebook with a chunk named "setup" (note that since this book is advocating the use of here() to set the file path, naming the setup chunk "setup" is not imperative).
...
library(here) # here::here() creates the absolute path to the file
```
...
```{r import-tests, echo=FALSE, eval=FALSE}
...
```{r dung-beetle-fig, echo=FALSE, eval=TRUE}
knitr::include_graphics("images/Dung_beetles_reduce_livestock-fig_1.png")
```
