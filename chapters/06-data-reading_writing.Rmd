...
In these examples, dt is the name of a data frame (that has been converted to a data table), not a function. Remember that all functions are followed by parentheses.
...
This "cleans" plant by omitting rows with any missing value

```{r}
plant <- na.omit(plant)
```

The problem is, any analysis of $shoot\_mass$ will not contain the value `round(plant[2, shoot_mass], 1) because it was deleted from the data frame along the rest of the row with the missing value.

As with subsetting, handle missing values at the level of the analysis by effectively using the subset of data with no missing values *for that analysis*. The function `is.na` returns a boolean (TRUE, FALSE) for each element in a list. Consequently, the function `!is.na` can be used to obtain the rows in which the value of $root_mass$ is "not na".
...
The `mean` and `sd` functions have NA handling methods. These are used in the script below Compare the results to the script using `!is.na`. Without using `!is.na`, the sample size for the control, *for the variable root_mass*, is incorrect--it is simply the number of rows where $Treatment$ is "cn". Consequently, the computation of the SE for root mass for the control is incorrect.
...
Familiarize yourself with these and with the NA handling methods of other functions such as `lm` and `predict`.
...
####  Vole data
This books uses data on the lifespans of voles given three different treatments (control, vitamin E supplementation, vitamin C supplmentation), from
...
```{r problem2, echo=FALSE, comment=FALSE, eval=TRUE}
...
```{r dung-beetle-fig, echo=FALSE, eval=FALSE}
dung_path <- here(image_path, "Dung_beetles_reduce_livestock-fig_1.png")
knitr::include_graphics(dung_path)
```
