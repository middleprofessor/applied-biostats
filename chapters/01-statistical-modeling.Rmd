# Linear models as statistical models
...
As with the equation for a line, the linear predictor part of a linear model is a function that maps a specific value of $X$ to a value of $Y$. This mapped value is the **expected value** given a specific input value of $X$. This is often written as $\mathrm{E}[Y|X]$, which is read as "the expected value of $Y$ given $X$". Importantly, $\mathrm{E}[Y|X]$ is the **conditional mean**, which is the *modeled* value of $Y$ for all observations in which $X$ takes some specific value $x$.
...
\end{equation}
%
read as "epsilon is distributed as Normal with mean zero and standard deviation sigma"). This line explicitly specifies the distribution of the error part. The error part of a linear model is a random "draw" from a normal distribution with mean zero and variance $\sigma^2$. Think of this as adding some random value to the expected value. 

The inputs to a linear model (the $X$ variables) have many names including "independent variables," "predictor variables,", "explanatory variables," "treatment variables," and "covariates". The output of a linear model (the $Y$ variable or variables if the model is multivariate) is the "dependent variable," "response," or "outcome." The $\beta$ in the linear model are model **parameters** and if a parameter is multiplied by an $X$ variable then it is a **coefficient** (for example, $\beta_1$ in model \@ref(eq:lm) is a coefficient) (there are additional parameters that are hidden from this specification of the linear model but show up in the specification below). The coefficients of the $X$ in a linear model ($\beta_1$ in model \@ref(eq:lm)) are often called "the effects" (so $\beta_1$ is the effect of $X_1$).
...
These two ways of specifying the model encourage slightly different ways of thinking about how the data (the response varible $Y$) were generated. The linear model way "generates" data by constructing $y_i$ from the $x_i$ and then adding random, normal error. The statistical model way "generates" data by simply drawing a random variable from a normal distribution with a specified mean and variance. The statistical model way explicitly defines all parameters, including the parameters of the linear predictor ($\beta_0$ and $\beta_1$), the conditional mean $\mu$ and the variance $\sigma$. The linear model way only defines the parameters of the linear predictor, and often these are referrred to as "the parameters" in the sense that there are not other parameters. Because of the explicit statement of all parameters, the statistical model way to define the linear model is more useful in that it is easily generalized to more complex models, including hierarchical models, generalized linear models, and Bayesian models. This is why I call it the "statistical model" way of specification.
