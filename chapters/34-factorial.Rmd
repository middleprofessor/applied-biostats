...
knitr::kable(coef(summary(m1)), digits=c(2,2,1,3))
```
...
### In a factorial model, there are multiple effects of each factor

There is no "effect" of factor A in an experiment with crossed A and B factors. Instead, there is an effect of factor A for each level of factor B. Or, using the variables of the urchin experiment, there is an effect of Temp for each level of CO2, so there is an effect of Temp when CO2 is 400 µAtm and an effect of Temp when CO2 is 1100 µAtm. Similarly, there is an effect of CO2 when Temp is 13 C and an effect of CO2 when Temp is 18 C. The effects of factor A at each level of factor B are sometimes called the "simple" effects. Another name could be the "conditional" effects, since the value of the effect of A is conditional on the level factor B.

The conditional effects are four of the pairwise contrasts of the $2 \times 2$ combination of treatment levels of the urchin data.

```{r factorial-conditional-effects, echo=FALSE}
m1 <- lm(Resp ~ Temp*CO2, data=urchin)
m1.emm <- emmeans(m1, specs=c("Temp","CO2"))
summary(contrast(m1.emm, method="revpairwise", adjust="none"), infer=c(TRUE, TRUE))
```

### The additive model

If an interaction effect is small, then it might be useful to estimate the effects of the two factors as if the interaction were equal to zero. The resulting coefficients for the two factors are averaged effects, where the average is over all the levels of the other factor (see below). This can be useful if theThis model is

\begin{equation}
Resp = \beta_0 + \beta_1Temp^+ + \beta_2CO2^+
\end{equation}

This is a **reduced model** because one of the terms has been removed from the model. This particular reduced model is often referred to as the **additive model**, since it excludes the interaction term, which is a *product* of other terms. The model coefficients of the reduced model are 
...
means_table <- urchin[!is.na(Resp), .(mean=mean(Resp), N=.N), by=.(Temp, CO2)]
co2_effect_18 <- (m1.emm[Temp=="18" & CO2=="1100", emmean] - m1.emm[Temp=="18" & CO2=="400", emmean])
co2_effect_13 <- (m1.emm[Temp=="13" & CO2=="1100", emmean] - m1.emm[Temp=="13" & CO2=="400", emmean])
co2_effect <- (co2_effect_13 + co2_effect_18)/2
```

In the original coefficient table, from the full model including the interaction, $b_1$ is the effect of temperature when CO2 has the reference (400 µAtm) value. Here, $b_1$ is the pooled effect of temperature, which is the average of the effects computed at each level of CO2.

\begin{equation}
b_1 = \overbar{Temp_{+}}
\end{equation}

In this table, the estimate of $Temp^+$ is It is okay to drop the interaction term to estimate a "simple" effect of each factor by "smoothing over" a small, non-additive (interaction) effect, but, for several reasons, one should not drop a term because $p>0.05$.
...
m1 <- lm(Resp ~ Temp + CO2, data=urchin)
coef(summary(m1))
knitr::kable(summary(contrast(emmeans(fit, specs=c("Temp", "CO2")),
                              adjust="none",
                              method="revpairwise"), infer=c(TRUE, TRUE)), digits=c(2, 2, 2, 0, 1, 1, 1, 3))
```
