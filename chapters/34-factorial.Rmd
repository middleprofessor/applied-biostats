...
The four conditional (simple) effects are computed using the arguments `simple="each"` and `combine=TRUE`.
...
```{r}
m1.effects <- rbind(data.table(m1.effects.1), data.table(m1.effects.2))
m1.effects
```
