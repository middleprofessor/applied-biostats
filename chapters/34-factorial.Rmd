...
gt_table_2 <-  tab_style(gt_table_2,
    style = cells_styles(bkgd_color  = "#CCCCCC"),
...
colnames(m1.mm) <- c("400 µAtm", "1100 µAtm", "contrast")
m1.mm <- cbind("Temp"=c("13 C", "18 C", "contrast"), m1.mm)
knitr::kable(m1.mm, digits=4, caption = "Condional (simple) effects of full factorial model fit to urchin data")
```
...
The average of the simple effects for a factor are the **marginal effects**, sometimes called the **main effects** (especially in ANOVA). Marginal effects can be useful for summarizing a general trend, but, like any average, might not be especially meaningful if there is large heterogeneity of the simple effects, which occurs when the interaction effect is large.

```{r factorial-main}
m1.main <- copy(m1.mm)
m1.main$Temp <- as.character(m1.main$Temp)
temp_marginal <- mean(unlist(m1.main[3, 2:3]))
co2_marginal <- mean(m1.main$contrast, na.rm=TRUE)
...
This is a **reduced model** because one of the terms has been removed from the model. This particular reduced model is often referred to as the **additive model**, since it excludes the interaction term, which is a *product* of other terms. The model coefficients of the additive model are 
...
The resulting coefficients of the additive model are **marginal effects** -- the effects averaged over all levels of the other factor. Marginal effects can be useful for summarizing a general trend, but, like any average, might not be especially meaninful if there is large large heterogeneity of the simple effects (that is, when the interaction effect is large).


In the original coefficient table, from the full model including the interaction, $b_1$ is the effect of temperature when CO2 has the reference (400 µAtm) value. Here, $b_1$ is the **pooled** effect of temperature, which is a weighted average of the effects computed at each level of CO2.
