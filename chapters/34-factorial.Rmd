...
2. (WT/Active - WT/Rest) -- This is the **control (or reference) contrast** -- it is what we know based on prior knowledge and what we want to compare the treatment effect to. This should be positive based on prior knowledge.

We need to control for the expected increase in 2 using the contrast:

3. (KO/Active - KO/Rest) - (WT/Active - WT/Rest) -- This contrast is the interaction effect. This **focal effect** is what we need to estimate the knockout-induced stimulation effect.

**Framing 2**: What is stimulation-induced TLR9-/- effect?

1. (KO/Active - WT/Active) -- effect of KO during muscle stimulation. This is our **treatment effect**. If TLR9 is necessary for glucose uptake, then this should be big and negative.
...
3. (KO/Active - WT/Active) - (KO/Rest - WT/Rest) -- This contrast is the interaction effect. This **focal effect** is what we need to estimate the stimulation-induced TLR9-/- effect.
...
# 1. (KO/Active - KO/Rest) # treatment
# 2. (WT/Active - WT/Rest) # control
# 3. (KO/Active - KO/Rest) - (WT/Active - WT/Rest) Interaction

exp2j_m1_planned <- contrast(
  exp2j_m1_emm,
  method = list(
    "(KO/Active - KO/Rest)" = c(ko_active - ko_rest),
...
```{r twoway-exp2j-plot, echo=TRUE, eval=TRUE, fig.width=small_width, fig.asp = harrell_ar, dev.args = list(type = "cairo-png")}
ggplot_the_model(
  exp2j_m1,
  exp2j_m1_emm,
  exp2j_m1_planned,
  ...
  contrast_rows = c(2,1,3),
  rel_heights = c(0.5,1),
)
```
```{r twoway-exp2j-altplot, echo=TRUE, eval=TRUE, , fig.width=small_width, fig.asp = response_ar, dev.args = list(type = "cairo-png")}
...
ggplot_the_response(
  exp2j_m1,
  exp2j_m1_emm,
  exp2j_m1_pairs[c(3,2,4),],
  palette = pal_okabe_ito_blue,
  legend_position = "bottom",
  y_label = "Glucose uptake\n(nmol per mg protein\nper 15 min)",
  y_pos = c(13.2, 13.6, 13.2)
)
```
...
```{r twoway-what-coefs-are, echo=FALSE, fig.width=std_width, fig.asp=std_ar, fig.cap = "The coefficients of a linear model with two crossed factors, explained."}
...
```{r twoway-something-different, echo=FALSE, fig.width=std_width, fig.asp=std_ar, fig.cap = "The interaction is something different, not more of the same. To get the observed difference (4), take the expected difference (3) and add something different (5)."}
# image_path <- here("images", "twoway-something-different.png")
# knitr::include_graphics(image_path)
...
```{r twoway-what-we-want, echo=FALSE, fig.width=std_width, fig.asp=std_ar, fig.cap = "Why the interaction effect is the stimulation-induced TLR9-/- effect"}
# image_path <- here("images", "twoway-what-we-want.png")
# knitr::include_graphics(image_path)
gg_what_we_want
```
...
exp3e_m1_ixn <- cbind(contrast = "interaction",
                        exp3e_m1_ixn[, -c(1:2)]) # remove cols 1:2
# combine
exp3e_m1_planned <- rbind(exp3e_m1_planned, exp3e_m1_ixn)
...
## Example 3: Estimation of synergy ("More than the sum of the parts") (Experiment 1a ABA data)
...
The example data come from Figure 1a, which is the effect of HAC and Indole on tissue concentrations of the hormone abcisic acid (ABA). The design is fully crossed with two factors, each with two levels: $\texttt{hac}$, with levels "HAC-" and "HAC+", and $\texttt{indole}$, with levels ("Indole-" and "Indole+").
...
```{r twoway-aba-import, echo=FALSE, warning=FALSE, message=FALSE}
...
exp1[treat == "Control", c("hac", "indole") := list("HAC-", "Indole-")]
exp1[treat == "HAC", c("hac", "indole") := list("HAC+", "Indole-")]
exp1[treat == "Indole", c("hac", "indole") := list("HAC-", "Indole+")]
exp1[treat == "HAC+Indole", c("hac", "indole") := list("HAC+", "Indole+")]

treat_levels <- c("Control", "HAC", "Indole", "HAC+Indole")
exp1[, treat := factor(treat, levels = treat_levels)]
...
### Fit the model
```{r  twoway-aba-m1}
exp1c_m1 <- lm(aba ~ hac * indole, data = exp1)
```
...
exp1c_m1_pairs %>%
  kable(digits = c(1,1,1,1,2,1,1,1,2,5)) %>%
  kable_styling()
```

```{r twoway-aba-ixn}
exp1c_m1_ixn <- contrast(exp1c_m1_emm,
                           interaction = c("revpairwise"),
                           by = NULL) %>%
  summary(infer = TRUE)
...
```{r twoway-aba-ixn-combined}
# Make the interaction table conform (same column names)
# to the planned table
exp1c_m1_ixn <- data.table(exp1c_m1_ixn)
colnames(exp1c_m1_ixn)[1:2] <- colnames(exp1c_m1_pairs)[c(2,1)]
exp1c_m1_ixn[, indole := "(Indole+)"]
exp1c_m1_ixn[, hac := "(HAC+)"]
exp1c_m1_ixn[, contrast := "ixn"]

# combine
exp1c_m1_pairs <- rbind(exp1c_m1_pairs, exp1c_m1_ixn)
...
```{r twoway-aba-plot, echo=FALSE, fig.width=std_width, fig.asp=harrell_ar, dev.args = list(type = "cairo-png")}
ggplot_the_model(
  fit = exp1c_m1,
  ...
  effect_label = "Difference in ABA",
  contrast_rows = "all",
  rel_heights = c(0.6,1),
)
```
...
```{r twoway-aba-alternaplot, echo=FALSE, fig.width=std_width, fig.asp=response_ar, dev.args = list(type = "cairo-png"), fig.cap = "An alternative plot for showing the estimate of synergy. Gray, dashed line is the expected mean of the HAC + Indole group if the interaction is zero."}
b <- emm_b(exp1c_m1_emm)

gg <- ggplot_the_response(
  fit = exp1c_m1,
  fit_emm = exp1c_m1_emm,
  fit_pairs = exp1c_m1_pairs[1:4,],
  palette = pal_okabe_ito_blue,
  legend_position = "bottom",
  y_label = "ABA\n(ng per g FW)",
  contrast_rows = "all",
  ...
  geom_bracket(
    ...
    label = paste0("synergy\n(interaction)\np = ",
                   fmt_p_value_rmd(exp1c_m1_pairs[5,"p.value"])),
    text.size = 3,
    text.hjust = 0,
    color = "black") +
...
# image_path <- here("images", "twoway-example3-alternaplot.png")
# ggsave(image_path,
#        width = small_width,
#        height = small_width*response_ar,
#        units = "in")


```

```{r twoway-alternaplot-plot, echo=FALSE, fig.width=std_width, fig.asp=response_ar, dev.args = list(type = "cairo-png"), fig.cap = "An alternative plot for showing the estimate of synergy. Gray, dashed line is the expected mean of the HAC + Indole group if the interaction is zero."}
# image_path <- here("images", "twoway-example3-alternaplot.png")
# knitr::include_graphics(image_path)
gg
```
...
$$
\begin{equation}
\texttt{aba} = \beta_0 + \beta_1 (\texttt{hac}_\texttt{HAC+}) + \beta_2 (\texttt{indole}_\texttt{Indole+}) + \beta_3 (\texttt{hac}_\texttt{HAC+}:\texttt{indole}_\texttt{Indole+}) +\varepsilon
(\#eq:factorial-full)
\end{equation}
$$
...
```{r twoway-synergy, echo=FALSE, fig.width=std_width, fig.asp=std_ar, fig.cap="Synergy is the bit needed to get to the HAC + Indole mean after adding the HAC effect and the Indole effect to the Control mean"}
# image_path <- here("images", "twoway-synergy.png")
# knitr::include_graphics(image_path)
gg_synergy
```
...
m1_pairs[1:2, contrast := paste(stimulation, contrast, sep = ": ")]
m1_pairs[3:4, contrast := paste(genotype, contrast, sep = ": ")]
...
m1_pairs %>%
  ...
  kable_styling() %>%
  row_spec(2, background = pal_okabe_ito[1])
```
...
```{r twoway-anova-explainer-plot, echo=FALSE, fig.width=std_width, fig.asp = std_ar, message=FALSE, fig.cap = "The main effect of stimulation in the ANOVA table tests the average of the conditional effects of stimulation. The average of the conditional effects is the difference between the marginal means of Active and of Rest."}
