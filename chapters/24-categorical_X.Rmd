...
1. This table is important for reporting treatment effects and CIs and for plotting the model. A contrast is a difference in means. Unlike the contrast table in Example 1, which had only two groups, This contrast table has more information than is in the coefficient table.
2. The column $\texttt{Contrast}$ contains the names of the contrasts specified in the `contrast()` function. Here, these contrasts are all those between pairs of levels in $\texttt{treatment}$ (this is "all pairwise contrasts"). With $k$ levels, there are $k(k-1)/2$ pairwise contrasts. Note that the name gives the direction of the difference.
3. The values in the column $\texttt{estimate}$ are the contrasts. These are the differences in the conditional means of the groups identified in the $\texttt{Contrast}$ column. These are the **effects** that we are interested in.
4. The value in the "SE" column is the standard error of the difference (SED) of each contrast. This SE is computed using the model standard deviation $\sigma$.
5. The values in the "lower.CL" and "upper.CL" columns are the bounds of the 95% confidence interval of the estimate. Remember (from Chapter \@ref(variability)) to think of this interval as containing potential values of the true parameter (the true difference in means between the two groups) that are reasonably compatible with the data. Don't think of the interval as having 95% probability of containing the true effect. Remember that a confidence interval applies to the procedure and not a parameter -- 95% of the CIs from hypothetical, replicate experiments that meet all the assumptions used to compute the CI will include the true effect.
...
Compare the *t* and *p* values from the three indpendent tests with the *t* and *p*-values from the linear model.
...
# so there is not digit designation for this column
kable(m1_coef, digits = c(2,3,3,5,2,2)) %>%
  kable_styling()
# explor other styles in the kableExtra package
```
...
```
Notes
1. The `specs` argument specifies the factor variables used to compute the modeled means. The specified variables must be in the model object (here, `m1`) passed to the function. Any variable passed to `specs` must be categorical. Do not pass numeric covariates here.
2. If there are two factor variables in the model, and both are passed to `specs`, then the modeled means of all combinations of the levels of the two variables will be computed. If only one factor variable is passed, then the marginal means (averaged over all levels of the missing factor) are computed. This will become more clear in the chapter "Models for two (or more) categorical X variables".
3. If there are continuous covariates in the model, the modeled mean is computed at the average value of the continuous covariates.
...
1. `method = "revpairwise"`, computes contrasts between all pairs of groups within the factor(s) specified in the "specs" argument of `emmeans`. I prefer "revpairwise" to "pairwise" because "revpairwise" orders the difference as non-reference minus reference.
2. [See the list](https://rdrr.io/cran/emmeans/man/emmc-functions.html){target="_blank"} of the different contrasts available with `method =`. Also, read the [comparisons and contrasts vignette](https://cran.r-project.org/web/packages/emmeans/vignettes/comparisons.html){target="_blank"} for more on `emmeans::contrast()`
