...
The plot below shows the LDL-C response in the placebo and drug groups, including the group means and 95% confidence intervals. The *p*-value is from the *t*-test of the effect of treatment using the model
...
The plot shows large overlap in LDL-C. The coefficient table is

```{r}
round(coef(summary(lm(ldlc~treatment, data=fd))), 3)
```

There "is no effect of the drug ($p = .478$)" is of course an incorrect interpretation of the hypothesis test of the estimate of $\beta_1$. A correct interpretation is, there is too much noise to say much about the size or direction of the effect.

LDL-C is strongly correlated with age and there is a large range in age among the Applied Bistats students. Consequently, age will contribute to a large fraction of the variance in LDL-C. If so, this age-related variance might be masking the effect of the drug. Here is a plot of LDL-C vs. age, with treatment assignment color coded as above. Remember, these are the exact same values of LDL-C as in the first figure.
...
where $ldlc.r$ is the set of residuals. The effect of the new drug on these residuals is

```{r echo=FALSE}
fit <- lm(ldlc ~ age, data=fd)
...
```{r echo=FALSE}
fit <- lm(ldlc ~ age + treatment, data=fd)
fit2 <- lm(ldlc ~ treatment + age, data=fd)
coef(summary(fit))
coef(summary(fit2))
...
## Adding covarates can decrease prediction error in predictive models
