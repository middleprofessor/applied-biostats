...
Regression to the mean happens when $\rho < 1$ and it's consequences increase as $\rho$ goes to zero. What is meant by "consequences"?

If the fat weight of the *ob/ob* group at baseline is 1.2 standard deviations smaller than that of the *+/+* group, and if $\rho=0$, then we'd expect the difference at the end to be zero. To get to zero difference at the end, the *ob/ob* mice would have to gain more fat weight than the *+/+* mice. Since the expectation of the mean difference at the end is zero the expectation of the change score *is bigger for the ob/ob mice than for the +/+ mice*. That is the expectation of the *difference* in change score is conditional on (or "a function of") the difference in fat weight at baseline.
...
## Best practices xxx: Use a covariate instead of normalizing a response
