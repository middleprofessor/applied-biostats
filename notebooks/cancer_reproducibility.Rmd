...
delta_value <- numeric(niter)
delta_ratio <- numeric(niter)
for(iter in 1:niter){
  fd <- data.table(
  ...
  fd[, rel:=ratio/cn_ref]
  delta_gapdh[iter] <- mean(fd[treatment=="cn", gapdh]) - mean(fd[treatment=="tr", gapdh])
  delta_value[iter] <- mean(fd[treatment=="cn", value]) - mean(fd[treatment=="tr", value])
  delta_ratio[iter] <- mean(fd[treatment=="cn", ratio]) - mean(fd[treatment=="tr", ratio])
  prob[iter, "cov"] <- coef(summary(lm(value ~ gapdh + treatment, data=fd)))["treatmenttr", "Pr(>|t|)"]
...
qplot(x=delta_gapdh, y=delta_value) + geom_smooth(method="lm")
qplot(x=delta_gapdh, y=delta_ratio) + geom_smooth(method="lm")

```

The one-sample t-test against a reference value of 1 has high type I error
The two-sample t-test of the normalized value has an nominal type I error but is conditionally biased on gapdh
